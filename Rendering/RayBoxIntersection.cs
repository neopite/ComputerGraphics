...
namespace ImageConverter.Rendering
{
    ...
    {
        public static Boolean RayIntersectsBox(IRay ray, Node node)
        {
            ...
            double lo = inv_dir.x * (node.boundingBox.minX - ray_pos.x);
            double hi = inv_dir.x * (node.boundingBox.maxX - ray_pos.x);

            double tmin = Math.Min(lo, hi);
            double tmax = Math.Max(lo, hi);
            
            double lo1 = inv_dir.y*(node.boundingBox.minY - ray_pos.y);

            double hi1 = inv_dir.y*(node.boundingBox.maxY - ray_pos.y);

            tmin  = Math.Max(tmin, Math.Min(lo1, hi1));
            tmax = Math.Min(tmax, Math.Max(lo1, hi1));
            
            double lo2 = inv_dir.z*(node.boundingBox.minZ - ray_pos.z);

            double hi2 = inv_dir.z*(node.boundingBox.maxZ - ray_pos.z);
